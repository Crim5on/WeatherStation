...
static uint16_t busyWaitWhile(TIM_HandleTypeDef *timerHandle,
		GPIO_TypeDef *port, uint16_t dataPin, const GPIO_PinState val) {
	uint16_t loopCount = 0;
	uint16_t timeStampStart = __HAL_TIM_GET_COUNTER(timerHandle);

	while ((HAL_GPIO_ReadPin(port, dataPin) == val)
			&& (loopCount < LOOP_UPPER_BOUND)) {
		; // wait
		loopCount++;
	}
...
static bool dht_protocol_performHandshake(TIM_HandleTypeDef *timerHandle,
		GPIO_TypeDef *port, uint16_t dataPin) {
 ...
 */
static bool dht_protocol_receive40bits(TIM_HandleTypeDef *timerHandle,
		GPIO_TypeDef *port, uint16_t dataPin, DHTdataSet *dataSet) {
...
bool dht_protocol_readData(TIM_HandleTypeDef *timerHandle, GPIO_TypeDef *port,
		uint16_t dataPin, DHTdataSet *dataSet) {
